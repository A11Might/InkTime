...
- **横竖构图自适应**：横图走「通栏照片 + 底部文字条」，竖图/方图走「照片靠左全高 + 右侧文字栏」，主体不裁丢（阈值 `SIDE_TEXT_MAX_ASPECT = 1.3` 可在 render.py 调）
...
- **像素级一致**：预览即最终推送画面（预抖动 + `ditherType: NONE` 的推送策略）

## 结构

```
app.py                 Flask：/api/stats /api/photos /api/thumb /api/render
render.py              Pillow 渲染器（铺满裁剪 → 自动对比度 → 1-bit 抖动 → 文字条）
