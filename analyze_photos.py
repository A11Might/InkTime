...
def _post_with_channel_fallback(
    ...
    for _ in range(n):
        ...
        try:
            try:
                if re.search(r"//(127\.0\.0\.1|localhost|\[::1\])[:/]", url):
                    # 本机回环地址不走系统代理（代理环境变量会把 localhost 请求拖挂）
                    with requests.Session() as local:
                        local.trust_env = False
                        resp = local.post(url, headers=headers, json=body, timeout=timeout)
                else:
...
TYPE_VOCAB = ["人物", "孩子", "猫咪", "宠物", "家庭", "旅行", "风景",
              "美食", "日常", "文档", "杂物", "其他"]


def normalize_type(raw) -> str:
    """把模型返回的类型切成独立标签：去重 + 按固定词表顺序排列。
    模型偶尔会把 type 返回成数组（如 ["人物","孩子"]），一并兼容。"""
    if isinstance(raw, (list, tuple)):
        raw = "/".join(str(t) for t in raw)
    parts = [t.strip("[]'\" ") for t in re.split(r"[/、，,;；]|\s{2,}", str(raw)) if t.strip("[]'\" ")]
    seen, out = set(), []
    for t in parts:
        if t not in seen:
            seen.add(t)
            out.append(t)
    known = [t for t in out if t in TYPE_VOCAB]
    known.sort(key=TYPE_VOCAB.index)
    extra = [t for t in out if t not in TYPE_VOCAB]
    return "/".join(known + extra) if (known or extra) else "其他"
...
def _process_one_photo(path: Path, city_resolver) -> dict | None:
    ...
    if exif_datetime and len(str(exif_datetime)) >= 10 and str(exif_datetime)[4:5] == ":":
        # EXIF 原生格式 2023:09:02 21:17:46 → 2023-09-02 21:17:46
        exif_datetime = str(exif_datetime).replace(":", "-", 2)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="分析照片：扫描文件夹 → 视觉模型分类/评分/写文案 → 存入数据库",
        epilog="示例：python3.11 analyze_photos.py ~/Pictures/InkTimePhotos --limit 5")
    parser.add_argument("folder", nargs="?", default=None,
                        help="要扫描的照片文件夹（省略则用 config.py 的 IMAGE_DIR）")
    parser.add_argument("--limit", type=int, default=None,
                        help="本次最多处理多少张（先小批量试跑推荐）")
    ...
    if args.folder:
        ...
        print(f"[INFO] 扫描目录（命令行指定）：{IMAGE_DIR}")
